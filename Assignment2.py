...
# [1] Define and implement the seven operators
# b_not, b_and, b_or, b_xor, b_nand, b_nor, b_xnor as 0 / 1booleanfunctions(not T / F).
# Consider all to be binary(degree2) functions except b_not which is unary(degree1):
...
def analyze_functions():
    functions = [f0_false, f1_nor, f2_converse_nonimpl, f3_negation, f4_nimply, f5_neg_q, f6_xor,
                 f7_nand, f8_and, f9_xnor, f10_q, f11_imply, f12_p, f13_converse, f14_or, f15_true]
    data = []

    for func in functions:
        results = analyze_boolean_table(func)
        row = [func.__name__, func_body(func), results[6], results[7], reverse_engineer(func)]
        data.append(row)
...
# [7] Write a function half_adder(x, y) that uses the functions created in Task [1]
# as "gates" and produces the s (sum) and c (carry) outputs per the table in the slides.
