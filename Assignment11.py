...
# Winter 2024
# Assignment 11 -  Graphs & Graph Algorithms
# Raphael Attiaala
...
# Define a function incidence_matrix that provides the incidence matrix for a graph
def incidence_matrix(matrix, dir):
    edges = list(edge_set(matrix, dir))
    n_edges = len(edges)
    n_vertices = len(matrix)
    print(n_edges, n_vertices)
...
# [10] Define a function draw_graph(graph) that draws a graph and saves it as an image file.
# See https://stackoverflow.com/questions/20133479/how-to-draw-directed-graphs-using-networkx-in-python
...
def is_eulerian(matrix):
    ...
    if dir:
        ...
        # in_degree_odd = [indegree(matrix, i) % 2 == 1 for i in range(n)]
        # return not (any(out_degree_odd) or any(in_degree_odd))
        return False
    else:
...
def do_graph(assn, description, matrix, dir):
    ...
    properties = [reflexive, irreflexive, anti_reflexive, symmetric, antisymmetric, asymmetric,
    ...
    print_vertex_properties("\n Vertext Properties for Graph", matrix)
    print()
...
def main():
    ...
    do_graph(assn, "Random Symmetric Graph of Size 8", matrix2, False)
    matrix3 = random_graph(10, True, .9)
    do_graph(assn, "Random Symmetric Graph of Size 10 with p = .9", matrix3, True)
