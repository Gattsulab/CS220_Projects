...
# [1] Define a function master_theorem(a, b, c)
# that determines the order of magnitude of the solution to a recurrence of the form T(n) = aT(n/b) + O(nc).
...
# and extracts the a. b, and c to pass into the function of the previous task.
# You can make any reasonable assumptions about the format of said recurrence.
...
# [4] Define a sample function f1, like the one for MergeSort.
# Try to use the one-line if/else (aka "ternary expression") as it will make it easier to capture the function content.
def f1(f, n):
...
# Functions f1 - f3 were done in class together; add seven more f4 - f10 D&C recurrences for homework.
...
# Time Complexity for D&C Long Int Multiplication
def f4(f, n):
...
def run_functions(n):
    ...
    run_function(functions, data, "Long Int Karatsuba", "3f(n/2)+n", f12, n)
...
def main():
    n = 1024
    assn = "Assignment8"
    data, functions = run_functions(256)
